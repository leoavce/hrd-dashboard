...
export async function renderYearSection({ db, storage, programId, mount, years }) {
  ...
  mount.innerHTML = `
    <section class="sec">
      <div class="sec-hd">
        <h3>년도별 페이지</h3>
        <div class="yr-filter" id="yrFilter">
          <button class="pill active" data-y="ALL">전체보기</button>
          ${years.map(y=>`<button class="pill" data-y="${y}">${y}</button>`).join('')}
        </div>
      </div>
      <div id="yrContainer"></div>
    </section>
  `;

  const container = mount.querySelector('#yrContainer');
  const filterBar = mount.querySelector('#yrFilter');

  function paint(targetYear='ALL'){
    const targetYears = targetYear==='ALL' ? years : [targetYear];
    container.innerHTML = targetYears.map(y=> oneYearBlock(y, data[y] || {})).join('');
    container.querySelectorAll('.see-detail').forEach(btn=>{
      btn.addEventListener('click', ()=>{
        const y = btn.dataset.year; const kind = btn.dataset.kind;
  ...
  filterBar.addEventListener('click', (e)=>{
    ...
    filterBar.querySelectorAll('.pill').forEach(b=> b.classList.remove('active'));
  ...
  function openDetail(kind, y, v){
    ...
    let content = '';
    if (kind==='content'){
      content = `<div class="preview"><pre>${esc(v?.content?.outline || '내용이 없습니다.')}</pre></div>`;
    } else if (kind==='budget'){
      ...
      content = `
        ...
        <ul class="bul">${insights.map(it=>`<li>${esc(it.title||'')}: ${esc(it.detail||'')}</li>`).join('')||'<li>없음</li>'}</ul>
      `;
    } else if (kind==='design'){
...
function oneYearBlock(y, v){
  const outline = (v?.content?.outline || '').split('\n').slice(0,6).map(s=>`<li>${esc(s)}</li>`).join('') || '<li>내용 미입력</li>';
  const budgetRows = (v?.budget?.items||[]).slice(0,6).map(it=>`
    <div class="row"><div>${esc(it.name||'항목')}</div><div>${fmt.format(Number(it.subtotal||0))} 원</div></div>
  `).join('') || '<div class="muted">항목 없음</div>';
  const s = v?.outcome?.surveySummary || {};
  const insights = (v?.outcome?.insights||[]).slice(0,4).map(it=>`<li>${esc(it.title||'')}</li>`).join('') || '<li>없음</li>';
  const assets = (v?.design?.assetLinks||[]).slice(0,8).map(u=>`<div class="thumb"><img src="${u}" /></div>`).join('') || '<div class="muted">자산 없음</div>';
...
function ensureStyles(){
  ...
  s.textContent = `
    .sec .sec-hd{display:flex;justify-content:space-between;align-items:center}
    .yr-filter{display:flex;gap:6px;flex-wrap:wrap}
    .pill{border:1px solid var(--ahn-line);background:#fff;border-radius:999px;padding:6px 10px;cursor:pointer;color:var(--ahn-text)}
    .pill.active{background:var(--ahn-primary-weak)}
    .yr{background:#fff;border:1px solid var(--ahn-line);border-radius:12px;padding:12px;margin:16px 0}
    .yr-cap{font-weight:800;color:var(--ahn-text);margin-bottom:8px}
    .yr-box{background:#fff;border:1px solid var(--ahn-line);border-radius:10px;padding:10px;margin-top:10px}
    .yr-hd{font-weight:700;margin-bottom:6px;color:var(--ahn-text)}
    ...
    @media (min-width:1000px){ .gal{grid-template-columns:repeat(8,1fr);} }
...
function table(headers, rows){
  return `
    <table class="x-table" style="width:100%;border-collapse:collapse">
      <tr>${headers.map(h=>`<th style="border-bottom:1px solid var(--ahn-line);padding:8px 10px;background:#f7fbff;text-align:left">${esc(h)}</th>`).join('')}</tr>
      ${rows.map(r=>`<tr>${r.map(c=>`<td style="border-bottom:1px solid var(--ahn-line);padding:8px 10px;background:#fff">${esc(c)}</td>`).join('')}</tr>`).join('')}
    </table>
  `;
}
function label(k){ return ({content:'교육 내용',budget:'교육 예산',outcome:'교육 성과',design:'교육 디자인'})[k]||k; }
